...
| AstrBot | `>=4.9.2,<5` |
| Python | 3.10 或更高版本 |
| 消息平台 | 主要针对 OneBot v11 / aiocqhttp 验证 |
| 网络 | 服务器需要能够访问学校统一认证与缴费平台 |
...
1. 在 AstrBot 管理面板中把自己设置为管理员。
2. 在插件配置中填写统一身份认证账号和密码，或者向机器人发送 `设置账密`。
3. 查询一次宿舍余额：
...
如果统一认证要求验证码，自动登录会停止并记录错误，需要人工处理认证状态。
...
历史记录只保留最近 30 天。统计图是运行时生成文件，不应提交到 Git 仓库。

## 常见问题

### 查询时提示认证失败

检查账号密码是否仍然有效，并确认服务器能访问学校认证站点。若统一认证出现验证码、异常确认页面或认证流程发生变化，自动登录可能无法继续。

### 设置预警后没有私聊

机器人可能无法向非好友发起私聊。请添加机器人为好友；如果预警是在群聊中设置的，插件会尝试在该群回退提醒。

### 电费统计只有一个点

同一宿舍两次记录间隔不足一小时不会重复写入。等待一小时后再次查询，或等待每日自动查询积累数据。
...
- 当前日期是否处于配置的寒暑假范围；
- AstrBot 与插件是否在执行时间持续运行；
- 是否已经使用 `设置预警` 添加宿舍；
- 日志中是否存在网络或认证错误；
- `alert_check_hour` 是否被修改。
...
- 不要把账号、密码、Cookie 或 `X-Token` 提交到仓库或发送给他人。
- AstrBot 当前插件配置没有专用密码密文类型，凭据会保存在服务器配置文件中；请限制配置目录和管理面板访问权限。
- 建议使用专门的低权限服务器账户运行 AstrBot，并及时更新服务器和插件依赖。
- 本插件不会执行缴费平台返回的任意 JavaScript，只解析受信任的同域登录跳转地址。
